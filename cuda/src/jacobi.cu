...
__global__ void jacobiOptimizedOnDevice(float* x_next, float* A, float* x_now,
    ...
    if (idx < Ni) {
        ...
        */
        ...
        // Tried to use loop-ennrolling, but also here this gives a wrong
        // result..
        /*
...
int main(int argc, char* argv[]) {
    ...
    if (kernel == 1) {
        ...
        for (k = 0; k < iter; k++) {
            if (k % 2)
                jacobiOnDevice<<<nBlocks, blockSize>>>(x_now_d, A_d, x_next_d,
                                                       b_d, Ni, Nj);
            else
                jacobiOnDevice<<<nBlocks, blockSize>>>(x_next_d, A_d, x_now_d,
                                                       b_d, Ni, Nj);
        ...
        for (k = 0; k < iter; k++) {
            if (k % 2)
                jacobiOptimizedOnDevice<<<nTiles, tileSize>>>(
                    x_now_d, A_d, x_next_d, b_d, Ni, Nj);
            else
                jacobiOptimizedOnDevice<<<nTiles, tileSize>>>(
                    x_next_d, A_d, x_now_d, b_d, Ni, Nj);
            // cudaMemcpy(x_now_d, x_next_d, sizeof(float)*Ni,
            // cudaMemcpyDeviceToDevice);
        }
    }
